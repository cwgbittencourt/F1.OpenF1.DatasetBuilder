...
def _setup_logging(log_dir: str) -> None:
    ...
    log_path = Path(log_dir) / "extended_experiments_comparison.log"
    logging.basicConfig(
        level=logging.INFO,
...
def _latest_run_for(experiment_id: str, task: str) -> pd.Series:
    ...
    if runs.empty:
        raise RuntimeError(f"Nenhum run encontrado para task={task}")
    return runs.iloc[0]


def _row_from_run(run: pd.Series, task: str) -> dict[str, object]:
    row: dict[str, object] = {
        "task": task,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Consolida comparativo dos experimentos extendidos."
    )
    ...
    for task in tasks:
        run = _latest_run_for(experiment_id, task)
        rows.append(_row_from_run(run, task))
    ...
    csv_path = artifacts_dir / "extended_experiments_comparison.csv"
    ...
    with mlflow.start_run(run_name="extended_experiments_comparison") as run:
        mlflow.set_tag("task", "extended_experiments_comparison")
